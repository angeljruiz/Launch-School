...
//   and stores the result back in the register. For example, if we start with
//    a stack of [3, 6, 4] (where 4 is the topmost item in the stack) and a
//     register value of 7, the MULT operation transforms the stack to [3, 6]
//      (the 4 is removed), and the result of the multiplication, 28, is left
//       in the register. If we do another MULT at this point, the stack is
...
//
// input is a string
// string could have mutliple commands
//
// interal array for stack
// int to hold register
//
// string => break into subcommands
// check for word commands then perform
// otherwise store value in stack

function minilang(command) {
  let stack = [];
  let register = 0;

  command = command.split(' ').forEach( subcommand => {
    switch (subcommand) {
      case 'PUSH':
        stack.push(register);
        break;
      ...
      case 'SUB':
        ...
        break;
      case 'MULT':
        register = parseInt(stack.pop()*register, 10);
        break;
      case 'DIV':
